...
class Topic:
    ...
    def post_copy_system_prompt(self, max_chars: int = 240,
        ...
        return (
            ...
            f'  {{"headline": "...", "summary": "..."}}\n\n'
            f"\"headline\": a punchy, plain-English headline UNDER 70 characters that "
            f"names the concrete thing the bill does — the kind of line that makes "
            f"someone stop scrolling. Strip ALL statute verbs and legalese ('Requiring', "
            f"'Prohibiting', 'Concerning', 'Relating to', 'An act to', 'An Act amending "
            f"the act of', 'Establishing') and never mention the statute, chapter, or "
            f"section a bill amends. Use a concrete noun phrase, not a full sentence "
            f"(e.g. 'Towns could pause new data-center construction for 18 months', "
            f"'Daily recess for elementary students'). Never echo a section or chapter "
            f"header verbatim.\n\n"
            f"\"summary\": one or two plain-text sentences, UNDER {max_chars} characters "
            f"total, that a non-lawyer instantly understands and finds worth reading. "
            ...
            f"reader's time. Lead with NEW substantive detail the headline does not "
            f"already convey (who must do what, what specifically changes, who is "
            f"affected, a key requirement, dollar threshold, penalty, time limit, or "
            f"effective date) — the specifics are what make it worth a scroll-stopper's "
