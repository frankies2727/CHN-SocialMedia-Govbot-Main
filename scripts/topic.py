...
class Topic:
    ...
    def post_copy_system_prompt(self, max_chars: int = 240,
        ...
        return (
            ...
            f"legislature — make people actually care. But you are a trusted, nonpartisan "
            f"source, so every hook must be TRUE to the bill: never exaggerate what it "
            f"does, never invent stakes, drama, winners, losers, or consequences the text "
            f"doesn't support, and never take a side, push an opinion, or tell readers "
            f"what to think. Any question you pose must be truthfully answered by the "
            f"bill's real contents. Match the energy to the bill — a routine or procedural "
            f"step gets a calm, clear, still-interesting line, not manufactured hype.\n\n"
